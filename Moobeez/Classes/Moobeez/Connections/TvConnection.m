...
- (id)initWithTmdbId:(NSInteger)tmdbId completionHandler:(ConnectionTvHandler)handler {
    ...
    if ([Cache cachedTVs][StringId(self.tmdbId)]) {
    ...
    self = [super initWithParameters:[NSDictionary dictionaryWithObject:@"credits" forKey:@"append_to_response"] completionHandler:^(WebserviceResultCode code, NSMutableDictionary *resultDictionary, NSError *error) {
        ...
        if (code == WebserviceResultOk) {
            ...
            [Cache cachedTVs][StringId(tmdbId)] = tmdbTv;
...
- (void)cachedResponse {
    ...
    self.customHandler(WebserviceResultOk, [Cache cachedTVs][StringId(self.tmdbId)]);
