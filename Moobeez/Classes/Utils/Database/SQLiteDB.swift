...
class SQLiteDB:NSObject {
    ...
    let DatabaseName = "MoobeezDatabase"
    let DatabaseVersion = "3.0.0"
    ...
    var CurrentDatabasePath: URL?
    {
        get
        {
            return AppDelegate.GroupPath?.appendingPathComponent(CurrentDatabaseName)
        }
    }
	...
	func openDB(copyFile:Bool = true) -> Bool {
		...
		let path = CurrentDatabasePath?.path ?? ""
		...
		if !(fm.fileExists(atPath:path)) && copyFile {
			...
			let from = (rp as NSString).appendingPathComponent(CurrentDatabaseName)
			do {
